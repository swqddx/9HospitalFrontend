...
const CornerStoneDisplay: React.FC<IProps> = (props) => {
  ...
  const showImg = () => {
    ...
    const current_img = `wadouri:${BASE_URL}${url}`;
    //  Load & Display`
    cornerstone.loadAndCacheImage(current_img).then(
      (image: any) => {
        ...
        // 设置元素视口
        const viewport = cornerstone.getDefaultViewportForImage(canvasImg.current, image);
        // 显示图像
  ...
  return <div ref={canvasImg} style={{ height: '380px', width: '600px', margin: 'auto' }} />;
};
